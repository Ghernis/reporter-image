...
def build_top10_consumption_barchart(
    ...
    *,
    ...
    ax.set_title("Top 10 licencias — consumo vs disponible")
...
def build_top10_evolution_chart(
    ...
    *,
    ...
    ax.set_ylim(0, 105)
    ax.set_title("Top 10 licencias — consumo en el tiempo")
...
def build_top10_evolution_matrix(
    ...
    *,
    ...
    for idx, name in enumerate(names[:10]):
        ...
        ax.plot(x_dt, y, color="#0451e4", linewidth=1.2, label="% capacidad")
        # Regression line (thin)
        slope, intercept = np.polyfit(x_numeric, y, 1)
        y_reg = slope * x_numeric + intercept
        ax.plot(x_dt, y_reg, color="#e67e22", linewidth=0.8, linestyle="--", alpha=0.9, label="tendencia")
        ax.set_ylim(0, 105)
    ...
    plt.suptitle("Top 10 licencias — % capacidad y tendencia (P.E. = projected exhaustion)", fontsize=10, y=1.02)
    fig.tight_layout()
...
def build_top10_assignments_removals_chart(
    ...
    *,
    ...
    ax.set_title("Top 10 licencias — assignments y removals (últimos 90 días)")
...
def build_company_license_stacked(
    ...
    *,
    ...
    ax.set_ylabel("Cantidad de licencias (consumido)")
    ax.set_title("Licencias por empresa — cantidad apilada por tipo")
...
def build_company_heatmap_pct(
    ...
    *,
    ...
    plt.colorbar(im, ax=ax, label="% consumo", shrink=0.8)
    ax.set_title("Licencias por empresa — % consumo (mapa de calor)")
    fig.tight_layout()
    out_path = Path(out_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(out_path, bbox_inches="tight", dpi=dpi)
...
def build_company_assignments_removals_stacked(
    ...
    *,
    ...
    ax.set_xticklabels(companies, rotation=30, ha="right")
    ax.set_ylabel("Cantidad")
    ax.set_title("Movimiento por empresa — assignments y removals (90d)")
    ax.legend(loc="upper right", fontsize=8)
    fig.tight_layout()
    out_path = Path(out_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
...
def build_company_scatter_assignments_removals(
    ...
    *,
    ...
    ax.set_title("Movimiento por empresa — assignments vs removals")
...
def build_sharepoint_stacked(
    ...
    *,
    ...
    ax.bar(x, billable, width, label="Facturable", color=_SHAREPOINT_BILLABLE)
    ...
    ax.set_title("Almacenamiento SharePoint por empresa — facturable, E5 entitlement, archive")
    ax.legend(loc="upper right", fontsize=8)
    fig.tight_layout()
    out_path = Path(out_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
...
def build_sharepoint_efficiency_bars(
    ...
    *,
    ...
    ax.set_xlabel("% eficiencia (almacenamiento usado / entitlement)")
    ax.axvline(x=100, color="gray", linestyle="--", linewidth=0.8)
    ax.set_title("Eficiencia de almacenamiento por empresa")
    fig.tight_layout()
    out_path = Path(out_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(out_path, bbox_inches="tight", dpi=dpi)
...
def build_sharepoint_archive_ratio_bars(
    ...
    *,
    ...
    ax.set_yticks(y_pos)
    ax.set_yticklabels(companies, fontsize=9)
    ax.set_xlabel("% ratio de archivo (archive / almacenamiento total)")
    ax.set_xlim(0, 100)
    ax.set_title("Ratio de archivo por empresa")
    fig.tight_layout()
    out_path = Path(out_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(out_path, bbox_inches="tight", dpi=dpi)
...
def build_sharepoint_trend(
    ...
    *,
    ...
    ax.set_xlabel("Fecha")
    ax.set_ylabel("Almacenamiento usado (GB)")
    ax.set_title("Evolución almacenamiento SharePoint por empresa")
...
def build_activity_buckets_barchart(
    ...
    *,
    ...
    ax.set_title("Actividad de usuarios (E5 / E3 / F3) — activos, bajo uso, inactivos")
    fig.tight_layout()
    out_path = Path(out_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(out_path, bbox_inches="tight", dpi=dpi)
...
def build_activity_breakdown_barchart(
    ...
    *,
    ...
    ax.set_title("Desglose de uso — usuarios por tipo de actividad")
    fig.tight_layout()
    out_path = Path(out_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(out_path, bbox_inches="tight", dpi=dpi)
